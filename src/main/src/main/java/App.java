...
public class App 
{
    ...
    private static void initializeTestData(EntityManager em) {
        ...
        Reservation res2 = new Reservation(
                ...
                now.plusDays(2).withHour(16).withMinute(0).withSecond(0).withNano(0),
                "Entretien"
        );
    ...
    private static void testAvailableRooms(SalleService salleService) {
        ...
        LocalDateTime start1 = now.plusDays(1).withHour(9).withMinute(0).withSecond(0).withNano(0);
        LocalDateTime end1   = now.plusDays(1).withHour(11).withMinute(0).withSecond(0).withNano(0);
        ...
        if (availableRooms1.isEmpty()) {
            ...
            availableRooms1.forEach(s ->
                    System.out.println("- " + s.getNom() + " (capacité: " + s.getCapacite() + ")")
            );
        }

        LocalDateTime start2 = now.plusDays(5).withHour(14).withMinute(0).withSecond(0).withNano(0);
        LocalDateTime end2   = now.plusDays(5).withHour(16).withMinute(0).withSecond(0).withNano(0);
    ...
    private static void testMultiCriteriaSearch(SalleService salleService) {
        ...
        salleService.searchRooms(criteria1).forEach(s ->
                System.out.println("- " + s.getNom() + " (capacité: " + s.getCapacite() + ")")
        );
        ...
        salleService.searchRooms(criteria2).forEach(s ->
                System.out.println("- " + s.getNom() + " (bâtiment: " + s.getBatiment() + ")")
        );
        ...
        salleService.searchRooms(criteria3).forEach(s ->
    ...
    private static void testPagination(SalleService salleService) {
        ...
        PaginationResult<Salle> result = new PaginationResult<>(firstPage, 1, pageSize, totalItems);

        System.out.println("\nInformations de pagination :");
        System.out.println("Page courante            : " + result.getCurrentPage());
        System.out.println("Taille de la page        : " + result.getPageSize());
        System.out.println("Nombre total de pages    : " + result.getTotalPages());
        System.out.println("Nombre total d'éléments  : " + result.getTotalItems());
        System.out.println("Page suivante disponible : " + result.hasNext());
        System.out.println("Page précédente dispo.   : " + result.hasPrevious());
    }
}
